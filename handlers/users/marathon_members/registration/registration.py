...
async def is_msk(callback: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if choice == 'yes':
        ...
        text = f"В ближайшие сутки с тобой свяжется ментор челленджа и подробнее расскажет как у нас всё устроено. " \
               f"Мини-коуч беседа (5-10 минут) является обязательным условием перед стартом, чтобы привычка " \
               f"приобреталась максимально комфортно и успешно.\n\n" \
               f"А пока лови ссылку на чат и знакомься с участниками. входе в чат, пожалуйста, представься и " \
               f"напиши 2-3 предложения о себе. Это важно для установления контакта с единомышленниками. " \
