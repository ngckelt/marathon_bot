...
async def bot_start(message: types.Message, state: FSMContext):
    ...
    if marathon_member is None:
        ...
        await message.answer("При ответе на вопросе будут использоваться кнопки. Телеграм устроен так, что панель с "
                             "кнопками иногда скрывается. Чтобы ее открыть нажмите в поле ввода иконку "
                             "с 4 квадратами внутри.")

        await message.answer_photo("AgACAgIAAxkBAAICyGFe5PlsTxOG0vryHDZy8"
                                   "UdK-lMrAAJLuTEbulTwSjtWkzjV9A22AQADAgADeQADIQQ")
...
async def instruction(message: types.Message, state: FSMContext, author=False):
    markup = create_markup(2, 'Попробовать 5 дней челленджа', 'Посмотреть отзывы')
    if author:
        markup = create_markup(2, 'Попробовать 5 дней челленджа', 'Посмотреть отзывы', 'Узнать о создателе челлендажа')
    await message.answer(
    ...
    await message.answer(
        ...
        "На самом деле это не все правила. Подробнее обо всём ты можешь узнать на пробных 5 днях челленджа.\n"
        "Ну что? Ты готов(а)?",
        reply_markup=markup
    )
