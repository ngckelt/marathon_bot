...
kick_marathon_member_callback = CallbackData('kick_marathon_member', 'member_telegram_id', 'accept')
...
def kick_marathon_member_markup(member_telegram_id):
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton(
            text="Засчитать отсчет",
            callback_data=kick_marathon_member_callback.new(member_telegram_id, "True")
        )
    )
    markup.add(
        InlineKeyboardButton(
            text="Не засчитывать отчет",
            callback_data=kick_marathon_member_callback.new(member_telegram_id, "False")
        )
    )
    return markup
